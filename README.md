Security Engineer.

Current Interests:
- Building slop filters for AI-assisted development.

### What I'm working on:

- **[arch-dev-cloud](https://github.com/reg1z/arch-dev-cloud)** — Reproducible Arch Linux dev environment deployable to GCP, AWS, Azure. A place to `--dangerously-skip-permissions` while minimizing blast radius.
- **[scap-project](https://github.com/reg1z/scap-project)** — SCAP/STIG compliance scanning showcase with Docker + OpenSCAP.
- 🔨 Currently building a SAST scanning tool for AI development workflows.

